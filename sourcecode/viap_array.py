			...
			if type(statement.lvalue) is c_ast.ID:
				if statement.lvalue.name in all_var_int:
					...
					if statement.lvalue.name in in_var_map.keys():
						update_statements.append(c_ast.Assignment(op='=', lvalue=in_var_map[statement.lvalue], rvalue=reconstructStmt(statement.rvalue,count,var_map,in_var_map,fun_count,all_var_int)))
					else:
						update_statements.append(c_ast.Assignment(op='=', lvalue=statement.lvalue, rvalue=reconstructStmt(statement.rvalue,count,var_map,in_var_map,fun_count,all_var_int)))
			else:
				update_statements.append(c_ast.Assignment(op='=', lvalue=statement.lvalue, rvalue=reconstructStmt(statement.rvalue,count,var_map,in_var_map,fun_count,all_var_int)))
		elif type(statement) is c_ast.While:
	...
	for stmt in assume_list:
		if stmt[0]=='e':
			...
			if stmt[0]=='s0':
				...
				if stmt[0]=='i1':
					stmt_assume=[]
					stmt_assume.append('s0')
					stmt_assume.append(stmt[4][2])
					update_assert_stmts.append(stmt_assume)
				else:
					update_assume_stmts.append(stmt)
